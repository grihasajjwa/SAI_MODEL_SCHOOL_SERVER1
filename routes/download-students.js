...
router.get('/download', async (req, res) => {
    try {
        ...
        worksheet.columns = [
            ...
            { header: 'Pickup Order', key: 'pickupOrder', width: 15 },
        ...
        students.forEach(student => {
          const row =  worksheet.addRow({
                ...
                tuitionFee: Number(student.tuitionFee || 0) || 0,
                address: student.address,
                transportRequired: student.transport?.required ? 'Yes' : 'No',
                transportFees: Number(student.transport?.fees || 0) || 0,
                transportStartDate: student.transport?.startDate || '',
                busNumber: student.transport?.busNumber || '',
                pickupPoint: student.transport?.pickupPoint || '',
                route: student.transport?.route || '',
                pickupOrder: student.transport?.pickupOrder || '',
